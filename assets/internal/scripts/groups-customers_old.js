var groupCustomers = function () {
    ...
    var initTable = function (groupList) {
        ...
        $('#customer-table_filter input[type=search]').keyup(function (e) {
        ...
        $('input[type=checkbox].select-all').click(function (e) {
            ...
            $.each(checkboxes, function (e) {
                ...
                form.push({name: customerBucket, value: $(this).data('customer-id')})
            });

            $.post(clientGroupCustomersURL, form)
                .done(function (response) {
                    var msg = "All " + checkboxes.length + " customers has been "+ (checked ? "added to ": "removed from ") + response.group + " group.";
        ...
        oTable.$('input[type=checkbox]').click(function () {
            ...
            form.push({name: customerBucket, value: customerId});
    ...
    var loadCustomerTable = function () {
        ...
        $.get(clientCustomerURL, {'distinct': ['customer_name']})
            .done(function (response) {
                ...
                $.get(clientGroupsURL)
                    .done(function (response) {
                        ...
                        $.each(customerList, function (i, customer) {
                            ...
                            var $tr = $('<tr>').append($('<td>').text(customer.customer_name));
                            //groups
                            if (groupList.length > 0) {
                                $.each(groupList, function (i, group) {
                                    var checked = ($.inArray(group.id, customer.groups) != -1 ? true : false);

                                    $tr.append($('<td>').prop('class', 'td-checkbox').append(
                                        $('<input>').prop('type', 'checkbox').prop('name', 'customers').prop("checked", checked).data('group-id', group.id).data('customer-id', customer.id).prop('class', 'group-item-'+group.id),
